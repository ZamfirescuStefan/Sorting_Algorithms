...
void is_sorted ( int v[], int n){
    for ( int i = 0; i < n - 1; i ++ )
        if ( v[i] > v[i + 1])
        {
            g << "The array is not sorted" << '\n';
            return;
        }
    g << "The array is sorted " <<"\n";
}
...
void tester(void(*f)(int v[], int len), int vct[], int n){
    ...
    g << "Sorting time: " << fixed << time_taken << setprecision(5) << " sec " << "\n";
    is_sorted(vct,n);
    g << "\n";
}
...
int main() {
    ...
    for ( int i = 0; i < nr_teste; i ++) {
        ...
        g << "Radix Sort sorting method:" << "\n";
        copiere(v1,v,len);
        tester(RadixSort, v1, len);

        g << "Quick Sort sorting method:" << "\n";
        copiere(v1,v,len);
        tester(QuickS,v1,len);

        g << "Merge Sort sorting method:" << "\n";
        copiere(v1,v,len);
        tester(MergeS,v1,len);

        if( nr_max <= 1000000){
            g << "Count Sort sorting method " << "\n";
            copiere(v1,v,len);
            tester(CountSort,v1,len);
        } else{
            g << "Too large numbers to use Count Sort:" << "\n" << "\n";
        }

        if (n <= 30000){
            g << "Bubble Sort sorting method " << "\n";
            copiere(v1,v,len);
            tester(bubble_sort,v1,len);
        } else
            g << "Too many numbers to use Bubble Sort:" << "\n" << "\n";

        if ( n <= 30000 ){
            g << "Insertions Sort sorting method" << "\n";
            copiere(v1,v,len);
            tester(InsertionS,v1,len);
        }
        else
            g << "Too many numbers to use Insertion Sort:" << "\n" << "\n";

        if ( n <= 30000 ){
            g << "SelectionSort sorting method" << "\n";
            copiere(v1,v,len);
            tester(SelectionSort,v1,len);
        }
        else
            g << "Too many numbers to use Selection Sort" << "\n" << "\n";

        g << "Sorting algorithm from C++: " << "\n";
        copiere(v1,v,len);
